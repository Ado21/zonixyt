...
class YouTubeScraper {
     ...
     */
    async getDownloadUrls(videoId, options = {}) {
        const {
            quality = '720',
            codec = 'h264',
            audioOnly = false,
        ...
        if (audioOnly) {
            ...
            if (!audioUrl) {
                if (!_retry) {
                    ...
                    return this.getDownloadUrls(videoId, {
                        ...options,
    ...
    async downloadVideoAndAudio(videoId, options = {}) {
        ...
        const data = await this.getDownloadUrls(videoId, { quality, codec, client });
    ...
    async downloadAudio(videoId, options = {}) {
        ...
        const data = await this.getDownloadUrls(videoId, { 
            audioOnly: true, 
            codec, 
            client 
        });
    ...
    async getInfo(videoId, options = {}) {
        const data = await this.getDownloadUrls(videoId, options);
        return data;
    }
}
